...
/*
This file is intended to use from matlab to acquire with the usb-dux dac*/
...
int main(int argc, char *argv[])
{
	...
	options.n_scan = atoi(argv[1]);
	options.freq = atof(argv[2]);
	options.n_chan = atoi(argv[3]);
	options.filename = argv[4];
	options.subdevice = atoi(argv[5]);
	options.channel = atoi(argv[6]);
	options.range = atoi(argv[7]);
	options.aref = atoi(argv[8]);
	options.physical = atoi(argv[9]);
